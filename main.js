...
// How many total commits were made in all of Steve's events? (githubData not defined?) commits is an array in an object in an array (githubData) commits line: 2378

let totalCommits = 0

for ( i = 0; i < githubData.length; i++) {
    if ("commits" in githubData[i].payload) {
    const allComm = githubData[i].payload.commits.length;
    totalCommits += allComm;
    }

}
console.log(totalCommits)
    
// How many of each event type are there? (PullRequestEvent, PushEvent, etc) PullRequestEvent, PushEvent, DeleteEvent, CreateEvent, IssueCommentEvent

let pullRequestEvent = "PullRequestEvent"
let pushEvent = "PushEvent"
let deleteEvent = "DeleteEvent"
let createEvent = "CreateEvent"
let issueCommentEvent = "IssueCommentEvent"

const eventFinder = (eventName) => {
  let totalEvents = 0;
  for (i = 0; i < githubData.length; i++) {
    if (eventName === githubData[i].type) {
      totalEvents++;
    }
  }
  console.log (eventName + totalEvents);
  }

eventFinder(pullRequestEvent);
eventFinder(pushEvent);
eventFinder(deleteEvent);
eventFinder(createEvent);
eventFinder(issueCommentEvent);
